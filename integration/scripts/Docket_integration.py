...
def matrix_comp(Mut_mat, drug_matrix):
    ...
    jacard_cof_r2r =  len(set(row_names_A).intersection(set(row_names_B))) / len(set(row_names_A + row_names_B) )
    jacard_cof_c2c =  len(set(col_names_A).intersection(set(col_names_B))) / len(set(col_names_A + col_names_B) )
    
    jacard_cof_r2c =  len(set(row_names_A).intersection(set(col_names_B))) / len(set(row_names_A + col_names_B) )
    jacard_cof_c2r =  len(set(col_names_A).intersection(set(row_names_B))) / len(set(col_names_A + row_names_B) )
...
def merge_matrix(Mut_mat,drug_matrix,direction_for_merge1,direction_for_merge2):
    ...
    if Label2_for_merge == 'Row' and Label2_for_merge == 'Row':
        result = pd.merge(Mut_mat,drug_matrix, left_index=True, right_index=True, how='inner')
    return(result)
...
def enrichment_forSignificance_res(result_annotate, label_1, value_1, label_2, value_2, p_threshold):
    ...
    list_fortest = [[x_res_EGFR_signal_sig.shape[0],x_res_EGFR_signal.shape[0] - x_res_EGFR_signal_sig.shape[0] ],[x_res_nonEGFR_signal_sig.shape[0], (x_res_nonEGFR_signal.shape[0]- x_res_nonEGFR_signal_sig.shape[0])]]
...
def enrichment_forSignificance_sen_all(result_annotate, F1, F2, p_threshold):
    ...
    for gene in list(set(result_annotate[F1])):
        for mol_action in list(set(result_annotate[F2])):
            p = enrichment_forSignificance_sen(result_annotate, F1, gene, F2, mol_action, p_threshold)
            if p < p_threshold:
                p_list.append(p)
                gene_list.append(gene)
                mol_action_list.append(mol_action)

    result = pd.DataFrame({'Gene':gene_list, 'mol_action':mol_action_list, 'p': p_list})
    return(result)
...
def enrichment_forSignificance_res_all(result_annotate, F1, F2, p_threshold):
    ...
    for gene in list(set(result_annotate[F1])):
        for mol_action in list(set(result_annotate[F2])):
            p = enrichment_forSignificance_res(result_annotate, F1, gene, F2, mol_action, p_threshold)
            if p < p_threshold:
                p_list.append(p)
                gene_list.append(gene)
                mol_action_list.append(mol_action)
...
def Annotation_feature(result, obj_id, Drug_annotation, list_forAnnotate, for_plot_F1,for_plot_F2, for_plot_x, for_plot_y):
    ...
    for i in temp:
        ...
        Annotation_list.append(Drug_annotation.loc[Drug_annotation[list_forAnnotate[0]] == i][list_forAnnotate[1]].values[0])
    ...
    fig = px.scatter(result, for_plot_x, for_plot_y, hover_data=[for_plot_F1,for_plot_F2,list_forAnnotate[1]],color= list_forAnnotate[1])
    fig.show()
    return(result)
