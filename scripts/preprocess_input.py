...
def main(file_configuration,
    ...
    assert isinstance(file_configuration, str)

    # Load configuration file for I/O
    io_config = io.load_io_config(file_configuration, base_directory)
    ...
    data_numeric.to_json(rows_data, orient='index')  # Row-wise
    data_numeric.to_json(cols_data, orient='columns')  # Column-wise
...
if __name__ == '__main__':
    ...
    # File IO arguments
    parser.add_argument('--file_config', help='Configuration file to load')
    parser.add_argument('--base_dir', help='Base directory', default=None)
    parser.add_argument('--fill_na', help='If true, fill missing numeric values', default=True)
